...
dbt platform connects to a Databricks SQL Warehouse to execute models. You need its HTTP path.
...
**Copy both values** — you will need them when creating the dbt platform connection in Part D.
...
dbt platform authenticates with Databricks using a PAT. OAuth is also supported but
...
Then open `finance/models/fct_revenue.sql` in dbt platform — it's 8 lines.
The question: "Which would you rather maintain?"

---

## Part D: dbt platform Setup (Platform + Mesh Consumers)

This step connects dbt platform to your Databricks workspace and builds all three
dbt projects: `platform` (the data producer), and `marketing` + `finance`
(the Mesh consumers). All three run in dbt platform — no local CLI required.

> **Plan note:** Cross-project Mesh refs require the **Team** or **Enterprise** plan
> in dbt platform. If you are on a trial, all features are available for 14 days.

---

### D1. Access your dbt platform account

1. Go to **cloud.getdbt.com**
2. Log in with your dbt Labs SSO (or your customer's dbt platform account)
...
1. In dbt platform, top-left account menu → **Account settings**
...
6. Click **Save** — dbt platform will test the connection. You should see a green checkmark.
...
dbt platform reads your project code from a git repository.

**Option A — GitHub (recommended):**
1. Fork or push `dbt-dbx-field-enablement/` to your GitHub account
2. In dbt platform → Account settings → **Integrations** → connect to GitHub
3. Authorize dbt platform to access your repositories

**Option B — GitLab / Azure DevOps:**
Same flow — dbt platform supports all major providers under Account settings → Integrations.
...
1. In dbt platform, top-left → **Account settings** → **Projects**
2. Click **New project**
3. Fill in:
...
| dbt version | `Latest` (select the Fusion engine / dbt Core v2.0 track) |
...
5. Next: set up an **Environment** for this project:
   ...
   - Name: `Production`
   - dbt version: `Latest` (inherit from project)
...
`{{ ref('platform', 'fct_orders') }}`. In dbt platform, this resolves via the
...
4. Set the project dependency:
   ...
   - This tells dbt platform: "when compiling `marketing`, fetch platform's published manifest"
   - Click **Save**
...
This is a key demo talking point. The three dbt platform projects are not just a
...
`marketing` project and explain that dbt platform enforces this at compile time —
...
> **2026 alternative — author metric views as dbt models.** With dbt-databricks
> 1.12+, a Unity Catalog metric view can be a governed dbt model
> (`materialized='metric_view'`). This repo ships one at
> `platform/models/metrics/orders_metric_view.sql`, which the platform build
> deploys automatically. Use that path when you want the metric views
> version-controlled, tested, and lineage-tracked; use the manual SQL below when
> you just want to stand up a metric view quickly for the comparison.
...
> Note: If your workspace supports the dbt Semantic Layer integration (dbt platform +
...
> approach works without dbt platform.
...
### dbt platform job fails: "Could not connect to Databricks"

1. In dbt platform → **Account settings** → **Connections** → `enablement-databricks`
...
4. If the warehouse was stopped, start it and re-run the dbt platform job
...
**In dbt platform:** the consumer projects resolve `{{ ref('platform', 'fct_orders') }}`
by fetching the platform project's published state from dbt platform's metadata service.
...
`--full-refresh` flag. In dbt platform → job → **Settings** → add `dbt run --full-refresh`
